...
import os
import pyodbc
print(" Available ODBC drivers inside container:", pyodbc.drivers())

conn_str = os.environ["DB_PATH"]
api_key = os.environ.get('API_KEY')

if not api_key:
    raise RuntimeError("API_KEY is not set!")

print("Fetched API key:", api_key[:6] + "..." if api_key else "None")
...
async def fetch_marketdata():
    ...
    try:
       ...
       marketCap = response.get("marketCap") if response else None
       volume = response.get("volume") if response else None
       btcDominance = response.get("btcDominance") if response else None
       marketCapChange = response.get("marketCapChange") if response else None    
       volumeChange = response.get("volumeChange") if response else None
       btcDominanceChange = response.get("btcDominanceChange") if response else None
    ...
    try:
      cursor.executemany('''INSERT INTO market_data (date, marketCap, volume, btcDominance,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''', [(date_time, marketCap, volume, btcDominance, marketCapChange, volumeChange , btcDominanceChange, fng_value, fng_name)])
...
async def fetch_coindata(url: str, coin: str, table_name: str):
 ...
 try:
   ...
   cursor.executemany(f'''INSERT INTO {table_name} (date, price, volume, marketCap,
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', [(date_time, price, volume, marketCap,
